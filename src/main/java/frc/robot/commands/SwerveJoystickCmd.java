...
public class SwerveJoystickCmd extends Command {
    ...
    public void execute() {
        ...
        // 取得最佳的 Limelight `tx`
        /*double bestTx;
        if (Vision.hasTargetUp()) {
            bestTx = Vision.getUpTx();
        } else if (Vision.hasTargetDown()) {
            bestTx = Vision.getDownTx();
        } else {
            bestTx = 0;
        }

        if (this.isAutoTurning.get()) {
            this.swerveSubsystem.autoTurning(this.xSpdFunction.get(), this.ySpdFunction.get(), bestTx);
        } else {
            this.swerveSubsystem.driveSwerve(this.xSpdFunction.get(), this.ySpdFunction.get(), this.turningSpdFunction.get());
        }*/
...
public class SwerveJoystickCmd extends Command {
    ...
    public void execute() {
        ...
        if(this.isAutoTurning.get()) {
            this.swerveSubsystem.autoTurning(this.xSpdFunction.get(), this.ySpdFunction.get(), Vision.getTx());
        } else {
            this.swerveSubsystem.driveSwerve(this.xSpdFunction.get(), this.ySpdFunction.get(), this.turningSpdFunction.get());
        }
